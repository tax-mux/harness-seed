...
fn user_config_path_uses_xdg_config_home() {
    ...
    let expected = dir.join("harness-seed").join("config.json");
    let got = crate::test_env::with_env("XDG_CONFIG_HOME", &dir, || user_config_path());
...
fn default_log_path_is_inside_user_config_dir() {
    ...
    let expected = dir.join("harness-seed").join("logs").join("events.jsonl");
    let got = crate::test_env::with_env("XDG_CONFIG_HOME", &dir, || default_log_path());
    assert_eq!(got, expected);
    let _ = fs::remove_dir_all(dir);
}
...
fn resolved_context_log_relative_uses_user_config_dir() {
    ...
    let expected = dir.join("harness-seed").join("logs").join("events.jsonl");
    let got = crate::test_env::with_env("XDG_CONFIG_HOME", &dir, || {
        let mut cfg = AppConfig::default();
        cfg.log.context_metrics = Some("logs/events.jsonl".into());
        cfg.resolved_context_log_path().expect("path")
    });
    assert_eq!(got, expected);
    let _ = fs::remove_dir_all(dir);
}
...
fn without_llm_max_tokens_env<F: FnOnce()>(f: F) {
    crate::test_env::without_all(
        &["HARNESS_SEED_LLM_MAX_TOKENS", "MYHARNESS_LLM_MAX_TOKENS"],
        f,
    );
}
