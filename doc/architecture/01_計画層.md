...
- 記憶層（ターン開始の recalled・diary）: [memory.md](../memory.md)
...
計画ループは短い。足りない証拠は実行層で集める。`answer` 未達時は一度だけ妥当な計画の出力を要求し、それでもダメなら `single_subtask(ユーザ要求)`（詳細は [memory.md](../memory.md) §3）。
