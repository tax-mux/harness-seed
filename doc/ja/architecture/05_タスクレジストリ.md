...
何度も同じ手順でやる作業を、**あらかじめ JSON の契約として置いておく置き場**。計画が「この契約を使え」と指すと、実行は LLM に毎回ツールを選ばせず、決まった手順（ステップドライバ）で進められる。

用語: [用語集.md](用語集.md)

## いつ使う／使わない

- 使う: 再現したい定型作業がある、引数や手順を機械的に監査したいとき
- 使わない: 毎回違う自由記述の作業だけなら、レジストリなしの freeform 実行で足りる

## 平易な流れ

`tasks/*.json` に手順を書く → 計画がタスク id を参照 → 実行がステップドライバまたは ReAct で走る → 監査
...
ツール選定・監査の詳細: [02-01_ツールの選択.md](02-01_ツールの選択.md) · English: [05_task-registry.md](../../en/architecture/05_task-registry.md)
