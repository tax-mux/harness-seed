...
実世界に効く最小の操作を **「ツールを 1 回呼ぶこと」** とみなす、という設計の前提。推論テキストや計画 JSON は「行動」ではなく、ツール実行とその結果（観察）がログやハーネスの基本単位になる。

用語: [用語集.md](用語集.md)

## いつ使う／使わない

- 使う: Action / Observation / Answer の境界を揃えたい、ログ設計をするとき
- 使わない: 層の役割だけ追うとき → [00](00_harness-seedの構造.md)

## 平易な流れ

考える → ツールを 1 回呼ぶ（行動）→ 結果を見る（観察）→ また考えるか、答える

関連:

- ReAct 実装（現行）: [08_ReAct実装.md](08_ReAct実装.md)
- アーキテクチャ索引: [README.md](README.md)
- コンテキストの層: [09_コンテキストマッピング.md](09_コンテキストマッピング.md)
- English: [10_agent-minimum-action-unit.md](../../en/architecture/10_agent-minimum-action-unit.md)
