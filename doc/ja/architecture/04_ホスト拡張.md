...
ホストが、エンジンの本筋（計画・実行・最終回答）を書き換えずに、**決まったタイミングで外部連携**（チケット起票・通知など）するための拡張面。そのコールバックを **hook** と呼ぶ。詳細な定義は §1。

用語: [用語集.md](用語集.md)

## いつ使う／使わない

- 使う: PM／チャット／課金など、ターン進行に合わせて副作用だけ載せたいとき
- 使わない: ツール選択や計画そのものを変えたいとき（それは本筋・設定・タスク契約の領域）

## 平易な流れ

ターン開始 → 計画確定（親チケットなど）→ 各作業の開始／終了 → ターン終了。袋（HostScratch）にチケット ID を置き、**LLM には見せない**。
...
- English: [04_host-extensions.md](../../en/architecture/04_host-extensions.md)
