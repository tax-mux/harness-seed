...
ホストが、エンジンの本筋（計画・実行・最終回答）を書き換えずに、決まったタイミングで外部連携するための拡張面である。そのコールバックを hook と呼ぶ。チケットや通知を本筋ループに混ぜると製品依存と再入で契約が壊れるので、副作用だけをホストに委ね、制御はエンジンが握る。hook から `run_turn` の再入や Answer／trace の書き換えはしない。

PM・チャット・課金など、ターン進行に合わせた副作用に使う。ツール選択や計画そのものを変えたい話は、本筋・設定・タスク契約の側である。ターン開始から計画確定、各作業の開始／終了、ターン終了までが主な地点。チケット ID などは HostScratch に置き、LLM には見せない。詳細な定義は次節。

実装: `src/lifecycle.rs` / `src/lifecycle/tracking.rs`。起動時の登録正本は `SeedBuilder`（`lifecycle(...)` → `build`、詳細は [06_ツールプラグイン.md](06_ツールプラグイン.md)）。実行中の差し替えは `set_lifecycle` / `lifecycle_from_tracking` / `seed_host_scratch`。用語: [用語集.md](用語集.md) · 方針: [development-principles.md](../development-principles.md) · [EN](../../en/architecture/04_host-extensions.md)

## 0. ホスト登録の正本（SeedBuilder）

埋め込みホストが覚える起動時の面は **`SeedBuilder` → `build()`** だけである。rules・タスク・ツールプラグイン・計画契約・lifecycle・memory をここに載せ、完成した `ReActLoop` を受け取る。

設定ファイルの場所もホストが決める。CLI の `--config` はバイナリ専用なので、ライブラリでは **`AppConfig::load_path` でホスト専用パスを渡す**のが推奨である（`load_default()` は XDG / env / cwd フォールバック。詳細は [config/README.md](../../../config/README.md#ライブラリ組み込み時のパス指定)）。

CLI や単体稼働は、ファイルや引数から資産を集めて同じ builder に流すヘルパーに留める（`merge_cli_agent` / `merge_agent_project`）。実行中にだけ足す変更は、これまでどおり `ReActLoop` の mutator を使う。
...
hook は、エンジンが進行の節目を通知してホストが外部処理を行う仕組みである。ツールは作業そのものを進め、その結果をモデルに返す。表示用の観測者は、どちらにも介入せず画面やログを見る。

したがってホストが決めるのは、どの節目でチケットや通知を更新するかである。次の作業や最終回答を決める権限はエンジン側に残す。
...
エンジンは計画と実行を進め、いつ通知するかを一貫して管理する。ホストは通知を受け、製品固有の API を使ってチケット、通知、課金などの外部状態を更新する。

この分担では、エンジンは特定の業務サービスを知る必要がない。ホストが同じ hook 面に接続するため、外部サービスを替えてもターンの制御は変わらない。
...
まずホストが業務用 API を実装し、それをライフサイクル hook としてエンジンへ登録する。ターンが始まると、エンジンは計画と実行を自分で進める。

節目ごとに hook が呼ばれ、ホストは外部システムを更新したり、ターン専用の袋に識別子を保存したりできる。その袋はモデルのプロンプトには渡らないため、業務上の内部情報が本筋の推論を変える経路にはならない。
...
エンジン側のライフサイクルは、ターン・計画・サブタスクという実行上の節目を表す。`TaskTracking` はそれらを、親子の work item を開始・更新・完了する業務上の操作へ対応付ける。

ホストはこの対応を実装するだけでよく、計画の内容を変更したり実行ループを再入させたりしない。成功・失敗・取消の結果もエンジンから渡されるので、外部側の状態を同じ終端へそろえられる。
...
ライフサイクル hook は、外部の状態を変えるための節目通知である。`TurnObserver` は画面やデバッグのために細かな進行を読むだけで、ホストの業務処理を担わない。

両方を登録しても役割は競合しない。前者が外部連携を完了させ、後者がその進行を見せる。
