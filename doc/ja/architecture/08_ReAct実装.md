...
「考えて →（必要なら）ツールを呼んで → 結果を見てまた考える」ループが、このリポジトリのコードでどう動くかのスナップショットである（2026-05 時点のソースに基づく）。層の役割説明だけではループ制限やステップ種別が追えないので、実装の境界を共有する。製品ドメインの手順解説ではない。

1 ユーザー入力は 1 ターン。`Answer` が返るまで Thought / Action / Observation を繰り返す。

用語: [用語集.md](用語集.md) · 単位: [10_最少行動単位.md](10_最少行動単位.md) · 索引: [README.md](README.md) · コンテキスト: [09_コンテキストマッピング.md](09_コンテキストマッピング.md) · ツール: [builtin_tools/README.md](../builtin_tools/README.md) · [EN](../../en/architecture/08_react-implementation.md)
...
REPL からの対話でも統合テストでも、入口は同じループに集まる。ループは次の判断を頭脳に求め、操作が必要なときだけツール実行部へ渡す。

この構成により、利用する頭脳をルールベースから LLM に替えても、ターンの進め方とツールの実行境界は変わらない。
...
ターンが始まると、ループは現在の入力と途中経過を頭脳へ渡して次の一手を受け取る。LLM を使う場合は、その呼び出しの使用量も同時に回収して記録する。

考えるだけなら trace に追加して次の判断へ進む。操作を選んだときはツールを実行し、結果を Observation として trace に戻す。Answer が返った時点で、ターン全体の計測をまとめて結果を返す。
...
3. `doc/ja/builtin_tools/<tool>.md` を追加し [README.md](../builtin_tools/README.md) の表に 1 行足す
...
| 項目 | 状態 |
|------|------|
| マルチターン会話メモリ | `SessionMemory` → `Previous turns`（**作業ログ経路のときだけ**）。外部記憶は [memory.md](03_記憶層.md)（Memory RAG + Bridge）。ターン内 `TurnTrace` は毎回新規 |
...
| [agent-minimum-action-unit.md](10_最少行動単位.md) | 最少行動単位の概念 |
