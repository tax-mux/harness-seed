...
実行中に **どの道具（ツール）を使ってよいか**を決めるルールの説明。候補の一覧 → この作業だけの制限 → 実行時の検査、の三段で絞る。

用語: [用語集.md](用語集.md)

## いつ使う／使わない

- 使う: 実行層の経路（ステップドライバ／ReAct）ごとに、使えるツールを理解・変更したいとき
- 使わない: 計画層（原則ツールなし）の話だけを追うとき

## 平易な流れ

カタログ（使える道具の一覧）→ サブタスクのポリシーでさらに制限 → 実際の呼び出しを検証
...
- English: [02-01_tool-selection.md](../../en/architecture/02-01_tool-selection.md)
