...
実行中にどの道具を使ってよいかを決めるルールである。カタログ全部を常に開けると誤選択や権限過大になりやすいので、候補の一覧 → この作業だけの制限 → 実行時の検査、と段階的に絞る。計画層にはツール実行を持ち込まない。

実行層の経路（ステップドライバ／ReAct）を触るときに読む。計画層だけを追う話ではない。

用語: [用語集.md](用語集.md) · 実行: [02_実行層.md](02_実行層.md) · タスク: [05_タスクレジストリ.md](05_タスクレジストリ.md) · 組み込み: [builtin_tools/README.md](../builtin_tools/README.md) · [EN](../../en/architecture/02-01_tool-selection.md)
...
最初に設定とホスト登録が、利用できる道具の上限を決める。次にサブタスクが定型の契約を持つかを見て、固定手順で進めるか、モデルに選ばせるかを分ける。

固定手順なら契約の順番だけを実行する。ReAct の場合でも、作業ごとの許可で候補を絞り、モデルには絞った一覧だけを見せる。実行時にも同じ許可を確認し、最後に実際の呼び出し履歴を契約と照合する。
...
候補集合は「存在する道具」の範囲を決める段階であり、個々の作業に使ってよい道具を決めるものではない。その次に経路を選び、定型なら契約が、自由な作業なら LLM が選択を担う。

LLM に選ばせる経路でも自由放任にはしない。サブタスクのポリシーで候補を減らし、実行時と事後監査で同じ制約を確かめる。
...
まずタスク定義に、必要なツール名と順番が書かれていることを確認する。条件を満たす場合はドライバがその定義を読み、指定された操作をそのまま実行する。

この経路では、LLM が候補から道具を選ぶ余地はない。引数だけをサブタスクの値で展開し、操作の結果は通常と同じ Action と Observation として記録する。
...
| 必須ツールの **呼び出し順序** | 実装済み |
| **禁止ツール**（deny）の使用 | 実装済み |
| 引数（期待 ⊆ 実引数） | `react.arg_audit_mode`: `soft`（既定・警告）/ `hard`（失敗）/ `off` |

未達時は監査メッセージ付き mission で ReAct 再試行（最大 2 回）。`soft` の引数警告だけでは再試行しない。

計画解決（`resolve_plan_with_tools`）では、必須 `method` が実行ツールレジストリに無い登録タスクを自由記述へ落とす。
...
| `react.arg_audit_mode` | 引数監査: `off` / `soft`（既定）/ `hard` |
