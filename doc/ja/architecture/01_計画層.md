...
ユーザーの依頼を読み、**何をどの順でやるか（サブタスク＝小さな作業の列）**を決める段階。ここではファイル操作などのツールは使わず、環境も書き換えない。「作業指示書を書く係」に近い。

用語: [用語集.md](用語集.md)

## いつ使う／使わない

- 使う: 依頼をそのまま実行に投げず、手順を一度固めてから動かしたいとき（標準の two_phase）
- 使わない対象ではないが、計画の中身をホストが全部決め切るなら、エンジンの計画 LLM は薄くできる

## 平易な流れ

依頼 →（ツールなしで考える）→ サブタスク列と要約 → 実行層へ渡す（または知識だけで答えて実行スキップ）

実装では ReAct 派生ループ（`run_plan_layer`）が動き、出力は `PlanArtifact`。
...
- 記憶層: [03_記憶層.md](03_記憶層.md)
- English: [01_planning-layer.md](../../en/architecture/01_planning-layer.md)
