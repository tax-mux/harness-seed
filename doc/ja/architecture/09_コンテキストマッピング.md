...
LLM に一度に渡す文章を、**何のために・どのくらい長く・どこに置くか**で整理した図解。短期の会話、今回思い出したこと、長い知識、などを混同しないための地図。

用語: [用語集.md](用語集.md)

## いつ使う／使わない

- 使う: プロンプトが肥大化した、何が LLM に見えているか切り分けたいとき
- 使わない: 記憶の取得経路だけ知りたいとき → [03_記憶層.md](03_記憶層.md)

## 平易な流れ

1 回の Chat リクエストを用途別に分解する（実装では主に `TurnPromptContext::render`）。
...
- English: [09_context-memory-mapping.md](../../en/architecture/09_context-memory-mapping.md)
