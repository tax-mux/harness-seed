...
CUI・GUI・別言語のホストが、エンジン（`ReActLoop`）と 1 行 1 JSON で会話するための約束である（プロトコルバージョン 1）。呼び出し形がホストごとに違うと組み込みコストが跳ねるので、同じ契約で多様なフロントから叩けるようにする。人間向けテキスト REPL（`--json` なし）の代替ではない。

ホストが JSON 1 行を送り、エンジンが処理し、JSON 1 行で返す。

用語: [用語集.md](用語集.md) · [EN](../../en/architecture/11_wire-protocol.md)
