...
計画で決めた作業を **実際に進め、必要ならツールで環境を変え、ユーザー向けの回答を出す**段階。計画層が「指示書」なら、実行層は「現場」である。

用語: [用語集.md](用語集.md)

## いつ使う／使わない

- 使う: 計画にサブタスクがあり、ツールや調査が必要なとき
- 使わない: 計画が「知識だけで足りる」と判断して実行スキップしたとき

## 平易な流れ

サブタスクを順に取る →（契約タスクなら手順どおり／そうでなければ LLM がツールを選ぶ）→ 結果をまとめ回答

主な経路は二つ: **ReAct ループ**（考えてツールを選ぶ）と **ステップドライバ**（契約どおり機械実行）。
