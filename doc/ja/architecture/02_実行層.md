...
| キー | 既定 | 実行層への影響 |
|------|------|----------------|
| `react.max_steps` | `16` | 1 サブタスクあたりの ReAct 上限。到達しても硬失敗せず、一度だけ answer を強制（失敗時は trace 根拠のフォールバック） |
...
| `react.two_phase` | `true`（CLI / `AppConfig`。`ReActConfig::default()` は `false`） | 計画→実行の直列（オフ時は単一ループ） |
| `react.advance.enabled` | `false` | フェーズ逐次実行 + `recalled` 引き継ぎ。証拠深化・主張監査・引用照合は [07_推進ループ.md](07_推進ループ.md) |
