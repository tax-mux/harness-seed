...
HarnessSeed が `ToolRuntime`（`src/tool/`）に実装しているツール。LLM 頭脳は各ターンで `{"step":"action","tool":"<name>","args":{...}}` として 1 回ずつ呼び出す。有効なツールは `tools.packs` で切り替える（[tool-plugins.md](../architecture/06_ツールプラグイン.md)）。
...
- [../architecture/08_ReAct実装.md](../architecture/08_ReAct実装.md) — ReAct ループ全体
- [../architecture/10_最少行動単位.md](../architecture/10_最少行動単位.md) — 最少行動単位（1 Action）
