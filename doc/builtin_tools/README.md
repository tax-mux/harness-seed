...
HarnessSeed が `ToolRuntime`（`src/tool.rs`）に実装しているツール。LLM 頭脳は各ターンで `{"step":"action","tool":"<name>","args":{...}}` として 1 回ずつ呼び出す。
...
- **実装**: `src/tool.rs` の `ToolRuntime::execute`
- **カタログ**: `tools_catalog()` — LLM への `Tool catalog` 追記用
