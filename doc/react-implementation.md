...
| ツール仕様書 | [builtin_tools/](builtin_tools/README.md)（実装は `src/tool.rs`） |
...
| `src/tool.rs` | ツール実行、`tools_catalog`（仕様は [builtin_tools/](builtin_tools/README.md)） |
...
- 利用可能ツールの列挙（[builtin_tools/](builtin_tools/README.md) / `tools_catalog()` と **両方**同期が必要。追加時は各ツールの `.md` も書く）
...
`ReActLoop` は `Action` を受け取ると `ToolRuntime::execute` を呼ぶ。実装の正本は `src/tool.rs`、人間向け仕様は **[builtin_tools/](builtin_tools/README.md)**（**1 ツール 1 ファイル**）。
...
1. `src/tool.rs` に `run_*` と `tools_catalog` 登録
...
| ツールの動的登録 | `tools_catalog` 固定（一覧は [builtin_tools/](builtin_tools/README.md)） |
