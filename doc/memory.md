# 記憶層（Memory RAG + MemoryBridge）

組み込みエージェントがホスト／外部記憶と結合するための実装正本。

- 開発方針: [development-principles.md](development-principles.md)（汎用解を優先）
- 設計経緯: [ideas/memory-and-replan-architecture.md](ideas/memory-and-replan-architecture.md)
...
| 層 | 責務 |
|----|------|
| **記憶 RAG**（`src/memory/rag.rs`） | ターン開始時に **作業ログか知識か** を分岐し、Bridge を叩いて `recalled` を組み立てる |
| **MemoryBridge** | `recent_work` / `search` / `diary` の I/O だけ。バックエンド固有ロジックはアダプタ内 |
| **計画・実行** | Bridge を直接知らない。`recalled` と `Previous turns` を消費するだけ |
